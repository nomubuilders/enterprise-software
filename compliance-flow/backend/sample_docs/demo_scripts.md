...
| 7 | Click **PostgreSQL** node → enter: `localhost` / `5433` / `compliance_db` / `compliance` / `&lt;your-password&gt;` / SSL off | *"Now I connect to our regulatory database"* |
...
| **DB Credentials** | `localhost` / `5433` / `compliance_db` / `compliance` / `&lt;your-password&gt;` / SSL off |
